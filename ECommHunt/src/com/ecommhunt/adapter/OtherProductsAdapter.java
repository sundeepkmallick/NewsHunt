...
public class OtherProductsAdapter extends BaseAdapter {
	private Context mContext;
	private ArrayList<Product> data;
	LayoutInflater mInflater;
	DisplayImageOptions options;

	public OtherProductsAdapter(Context c, ArrayList<Product> basicList) {
		mContext = c;
		data = basicList;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (stockQuantity > 0) {
			...
			grid.txtStockAvailable.setText(Utils.getStringDataBasedOnLanguage(mContext, R.string.in_stock));
			grid.txtStockAvailable.setTextColor(mContext.getResources()
					.getColor(R.color.txt_black));
		} else {
			grid.txtStockAvailable.setText(Utils.getStringDataBasedOnLanguage(mContext, R.string.out_of_stock));
			grid.txtStockAvailable.setTextColor(mContext.getResources()
		...
		grid.btnBuyNow.setText(Utils.getStringDataBasedOnLanguage(mContext, R.string.add_to_bag));
			...
			public void onClick(View v) {
				...
				if (totalItemsTobeAdded > 0) {
					if (DBOperations.addMultipleItemToBag(
						...
						Toast.makeText(
								mContext,
								data.get(position).getProductName() + " ",
	...
	private int totalItemsTobeAdded(String skuId, String stockQuantityStr) {
		...
		if (totalItemsTobeAdded >= stockQuantity) {
			...
			Toast.makeText(mContext,
					"Quantity selected is more than available stock",
					Toast.LENGTH_SHORT).show();
		}
